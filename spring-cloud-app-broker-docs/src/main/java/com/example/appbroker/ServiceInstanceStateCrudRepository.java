...
interface ServiceInstanceStateCrudRepository extends ReactiveCrudRepository<ServiceInstance, Long> {
	...
	Mono<ServiceInstance> findByServiceInstanceId(@Param("service_instance_id") Cadenas serviceInstanceId);

	@Query("delete from service_instance where service_instance_id = :service_instance_id")
	Mono<Void> deleteByServiceInstanceId(@Param("service_instance_id") Cadenas serviceInstanceId);
